...
class WakeWordFeatureExtractor:
    """
    EXACT copy of your training feature extractor for consistency.
    This ensures 100% matching between training and inference.
    """
    def __init__(self, 
                 sample_rate=16000,
                 n_mels=80,
...
class WakeWordEngine:
    def __init__(
        ...
        silent: bool = False  # New parameter to control logging output
    ):
        if model_path is None:
            # Default to models/callina.pt relative to project root
            model_path = str(Path(__file__).parent.parent / "models" / "callina.pt")
    ...
    def setup_audio(self) -> bool:
        ...
        try:
            ...
            if device_info:
                self.log(f"🎤 Using audio device: {device_info['name']}")
            ...
            self.log(f"❌ Failed to setup audio: {e}", "error")
            return False
            
    def _audio_callback(self, in_data, frame_count, time_info, status):
        """PyAudio callback for continuous audio capture."""
        if status and not self.silent:
            self.log(f"Audio callback status: {status}", "warning")
    ...
    def preprocess_audio(self, audio_data: np.ndarray) -> torch.Tensor:
        ...
        try:
            ...
            if features.shape[1] != 186:
                self.log(f"❌ Feature mismatch! Got {features.shape[1]}, expected 186", "error")
                return None
            
            # Debug logging (occasional) - only if not silent
            if len(self.confidence_history) % 200 == 0 and not self.silent:
                self.log(f"🔧 Features shape: {features.shape}")
                self.log(f"   Feature stats - mean: {features.mean():.4f}, std: {features.std():.4f}")
                self.log(f"   Feature range: [{features.min():.4f}, {features.max():.4f}]")
            
            return features
            
        except Exception as e:
            self.log(f"❌ Preprocessing error: {e}", "error")
            return None
    ...
    def detect_wake_word(self, audio_tensor: torch.Tensor) -> tuple[bool, float]:
        ...
        try:
            ...
            self.log(f"❌ Error during inference: {e}", "error")
            return False, 0.0
            ...
            try:
                ...
                if audio_tensor is None:
                    ...
                    if consecutive_errors >= max_consecutive_errors:
                        self.log("❌ Too many preprocessing errors, stopping", "error")
                        break
                    time.sleep(0.1)
                    continue
                ...
                consecutive_errors += 1
                self.log(f"❌ Error in detection worker: {e}", "error")
                if consecutive_errors >= max_consecutive_errors:
                    self.log("❌ Too many consecutive errors, stopping detection", "error")
                    break
                time.sleep(0.5)
                
        self.log("🛑 Detection worker stopped")
        
    def start(self) -> bool:
        """Start the wake word engine."""
        if self.is_running:
            self.log("⚠️ Engine is already running", "warning")
            return False
            
        self.log("🚀 Starting Wake Word Engine...")
        ...
        self.log("✅ Wake Word Engine started successfully!")
        self.log(f"🎯 Listening for wake word with confidence threshold: {self.confidence_threshold}")
        return True
    ...
    def stop(self):
        ...
        self.log("🛑 Stopping Wake Word Engine...")
        self.is_running = False
